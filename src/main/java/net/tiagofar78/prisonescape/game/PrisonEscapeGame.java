...
//	########################################
	...
	private void arrestPlayer(PrisonEscapePlayer arrested, PrisonEscapePlayer arrester) {
		...
		for (PrisonEscapePlayer player : _playersOnLobby) {
			MessageLanguageManager messages = MessageLanguageManager.getInstance(player.getName());
		...
		BukkitScheduler.runSchedulerLater(new Runnable() {
			...
			public void run() {
				...
				MessageLanguageManager messages = MessageLanguageManager.getInstance(arrested.getName());
